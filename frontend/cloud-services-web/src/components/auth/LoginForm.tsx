...
export function LoginForm() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      if (result?.error) {
        setServerError("Tên đăng nhập hoặc mật khẩu không chính xác. Vui lòng kiểm tra lại.");
        toast.add({
    ...
    <form noValidate onSubmit={handleSubmit(onSubmit)} className="p-6 md:p-8">
      <FieldGroup>
        <div className="flex flex-col items-center gap-2 text-center">
          <h1 className="text-2xl font-bold font-heading">Chào mừng bạn trở lại</h1>
          <p className="text-sm text-balance text-muted-foreground">
            Đăng nhập vào tài khoản CloudServices của bạn
          </p>
        </div>
        ...
        <Field>
          <div className="flex items-center">
            ...
            <Link
              href="/quen-mat-khau"
              className="ml-auto text-xs text-primary hover:underline underline-offset-2"
            >
              Quên mật khẩu?
            </Link>
          </div>
