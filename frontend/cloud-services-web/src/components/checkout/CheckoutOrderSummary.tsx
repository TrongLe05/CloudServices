...
export function CheckoutOrderSummary({
  ...
  return (
    <Card className="p-6 md:p-8 rounded-3xl bg-white border border-slate-200/90 shadow-sm space-y-6 sticky top-24">
      <CardHeader className="p-0 space-y-1">
        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
          Tóm tắt đơn hàng
        </span>
        <CardTitle className="text-lg font-bold text-slate-900 font-heading mt-0.5">
        ...
        {selectedPlan?.categoryName && (
          <div>
            <Badge
              variant="secondary"
              className="text-[10px] mt-1 font-semibold text-slate-600 bg-slate-100"
            >
              {selectedPlan.categoryName}
            </Badge>
          </div>
      ...
      <CardContent className="p-0 space-y-6">
        ...
        <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-100 text-[11px] text-slate-500 space-y-1.5">
          ...
          <p className="leading-relaxed">
            Khởi tạo tự động sau khi thanh toán. Cam kết SLA 99.99% Uptime và hỗ
            trợ kỹ thuật 24/7/365.
          </p>
        </div>
      </CardContent>
      ...
      {currentStep === 2 && (
        <CardFooter className="p-0">
          ...
          >
            <span>
              {isSubmitting ? "Đang xử lý tạo đơn..." : "Hoàn tất đặt hàng"}
            </span>
