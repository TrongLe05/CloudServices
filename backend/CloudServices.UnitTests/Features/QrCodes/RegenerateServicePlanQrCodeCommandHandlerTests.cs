...
public class RegenerateServicePlanQrCodeCommandHandlerTests
{
    ...
    {
        // Arrange
        var handler = new RegenerateServicePlanQrCodeCommandHandler(_repositoryMock.Object, _qrGeneratorMock.Object);
    ...
    public async Task Handle_PlanFound_GeneratesQrCodeAndReturnsDto()
    {
        ...
        _qrGeneratorMock.Setup(g => g.GenerateQrCode(It.Is<string>(url => url.Contains(planId.ToString()))))
            .Returns(fakeQrBytes);
        ...
        _qrGeneratorMock.Verify(g => g.GenerateQrCode(It.Is<string>(url => url.Contains(planId.ToString()))), Times.Once);
    }
}
